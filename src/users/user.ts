  ...
  _user.post('/sendMessage', async (req, res) => {
    ...
    try {
      ...
      for (let i = circuit.length - 1; i >= 0; i--) {
        ...
        encryptedMessage = await symEncrypt(
          symKey,
          `${destination}${encryptedMessage}`
        );
        const encryptedSymKey = await rsaEncrypt(
          await exportSymKey(symKey),
          node.pubKey
        );
        encryptedMessage = `${encryptedSymKey}${encryptedMessage}`;
        destination = (
          BASE_ONION_ROUTER_PORT +
          (i > 0 ? circuit[i - 1].nodeId : destinationUserId)
      ...
      res
        .status(200)
        .send({ message: 'Message sent successfully through the network.' });
    } catch (error) {
      console.error(error);
      res
        .status(500)
        .send({ error: 'Failed to send message through the network.' });
    }
  });
...
export async function forwardMessageToNode(
  node: { nodeId: number },
  message: string
): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to forward message to node ${node.nodeId}. Status: ${response.status}`
    );
  }
}
